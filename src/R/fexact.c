...
/* Modifications made to R code to make it build with gcc-rs. */

/* RBoolean */
#include "Boolean.h"

/* NORET from Rinterface.h */
/* Just ignore. */
#define NORET

/* R_Alloc (adapted from code in memory.c). */
#include <stdlib.h>
...
extern double r_fishers_exact(int* matrix_2x2, int tails) {
	// Ignore most parameters, and focus on the non-chi-squared,
	// 2x2 variant.
	int nrow = 2;
	int ncol = 2;
	int stride = 2;
	int workspace = 200000; // default.
	double expect = 0.0;
	double percnt = 0.0;
	double emin = 0.0;
	double prt = 0.0;
